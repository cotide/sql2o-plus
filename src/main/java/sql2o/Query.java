...
import sql2o.data.LazyTable;
import sql2o.data.Row;
import sql2o.data.Table;
import sql2o.data.TableResultSetIterator;
import sql2o.logging.LocalLoggerFactory;
import sql2o.logging.Logger;
import sql2o.quirks.Quirks;
import sql2o.reflection.PojoIntrospector;
...
public class Query implements AutoCloseable {
    ...
    private void addParameterInternal(String name, ParameterSetter parameterSetter) {
        if (!this.getParamNameToIdxMap().containsKey(name)) {
            throw new Sql2oException("Failed to add parameter with name '" + name + "'. No parameter with that name is declared in the query.");
        }
     ...
     *      .executeAndFetch(...)
     * </pre>
     * will generate the query : <core>SELECT * FROM user WHERE id IN(4,5,6)</core><br>
     * <br>
     * It is not possible to use array parameters with a batch <core>PreparedStatement</core>:
     * since the text query passed to the <core>PreparedStatement</core> depends on the number of parameters in the array,
     * array parameters are incompatible with batch mode.<br>
     * <br>
     * If the values array is empty, <core>null</core> will be set to the array parameter:
     * <core>SELECT * FROM user WHERE id IN(NULL)</core>
     *
    ...
    public <V> V executeScalar(Class<V> returnType){
        try {
            ...
            converter = Convert.throwIfNull(returnType, getQuirks().converterOf(returnType));
            //noinspection unchecked
            logExecution();
     ...
     * before implicitly calling <core>executeBatch()</core> from <core>addToBatch()</core>. <br/>
     *
     * When set to 0, executeBatch is not called implicitly. This is the default behaviour. <br/>
     *
     * When using this, please take care about calling <core>executeBatch()</core> after finished
     * adding all commands to the batch because commands may remain unexecuted after the
     * last <core>addToBatch()</core> call. Additionally, if fetchGeneratedKeys is set, then
    ...
    /**
     * Adds a set of parameters to this <core>Query</core>
     ...
     *
     * The current number of batched commands is accessible via the <core>getCurrentBatchRecords()</core>
     * method.
     */
     ...
     *
     * If maxBatchRecords is more than 0, executeBatch is called upon adding that many
     * commands to the batch. This method will return any generated keys if <core>fetchGeneratedKeys</core> is set. <br/>
     *
     * The current number of batched commands is accessible via the <core>getCurrentBatchRecords()</core>
     * method.
     */
